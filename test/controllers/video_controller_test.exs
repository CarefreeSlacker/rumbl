defmodule Rumbl.VideoControllerTest do
  ...
  alias Rumbl.Video
	...
	setup %{ conn: conn } = config do
		if username = config[:login_as] do
			user = insert_user(username: username, name: username)
  ...
  test "Requires user authentication", %{ conn: conn } do
	...
	test "lists all user's videos on index", %{ conn: conn, user: user } do
		first_video = insert_video(user, %{ title: "first_video" })
		second_video = insert_video(user, %{ title: "second_video" })
		conn = get conn, video_path(conn, :index)
		assert html_response(conn, 200) =~ "Listing videos"
		assert String.contains?(conn.resp_body, first_video.title)
		assert String.contains?(conn.resp_body, second_video.title)
	end

	@tag login_as: "max"

	test "authorizes actions against access by other users", %{ user: owner, conn: conn } do
		video = insert_video(owner, %{ title: "rebel", description: "description", url: "http://youtube.com"})
		non_owner = insert_user(username: "Other", name: "Other")
